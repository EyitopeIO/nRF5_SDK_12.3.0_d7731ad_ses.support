...
uint8_t println_buffer[USER_BUFFER_LEN] = {0};
...
uint8_t* adafruit_get_cleared_buffer(void)
{
  ...
  return println_buffer;
}

void adafruit_print(void)
{
  clear_screen(true);
  adafruit_print(println_buffer);
}

void adafruit_start_refresh_timer(void)
{
  uint32_t err_code = app_timer_start(m_refresh_display_timer_id, APP_TIMER_TICKS(1000,0), nullptr);
  APP_ERROR_CHECK(err_code);
}

void adafruit_stop_refresh_timer(void)
{
  uint32_t err = app_timer_stop(m_refresh_display_timer_id);
  APP_ERROR_CHECK(err);
}